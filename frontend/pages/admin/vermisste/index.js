...
function Vermisste() {
    ...
    const fetchData = async() => {
        try {
            const { data } = await BackendApi.get("/missing/unapproved?admin=1")
            ...
            if (data.missingPersons?.length > 0) {
                setMissingPerson({
                    ...defaultPerson, 
                    ...data.missingPersons[0],
                    appearance: data.missingPersons[0].appearance.join(" | ")
                })
            }
        } catch (error) {
    ...
    const approvePerson = async () => {
        try {
            ...
            const { data } = await BackendApi.put(`/missing/${missingPerson._id}`, modifiedPerson)
    ...
    const deletePerson = async() => {
        try {
            const { data } = await BackendApi.delete(`/missing/${missingPerson._id}`)
    ...
    const convertToWantedPerson = async() => {
        try {
            const { data } = await BackendApi.post(`/missing/${missingPerson._id}/convert`)
    ...
    const nextPerson = async() => {
        const updatedPersons = missingPersons.slice(1)
    ...
    return (
        <AdminLayout title="Ungeprüfte Vermisste">
            <h1 className="text-lg font-medium mb-2 text-center">Vermisste</h1>
            {loading ? (
                <p>Laden...</p>
            ) : missingPersons.length > 0 ? (
                <div className="mx-auto space-y-4 max-w-4xl">
                    <p><span className="text-light-gray">ID:</span> {missingPerson._id}</p>
                ...
                <p>Keine ungenehmigten Vermissten vorhanden.</p>
            )}
        </AdminLayout>
    );
}

export default Vermisste;
