...
export default function Aktuelles({ news, query, pagination }) {
    ...
    return (
        <Layout 
            title="Aktuelle Polizeimeldungen" 
            ...
            canonical={`${process.env.NEXT_PUBLIC_FRONTEND_URL}/aktuelles`}
        >
            ...
            {news.length > 0 ? (
                <div className="container mx-auto mt-10 flex gap-2 justify-center">
                    {pagination.currentPage !== 1 ? (
                        <Link href={`/aktuelles${getNewSearchParams(filter, pagination.currentPage - 1)}`} className="w-8 h-8 hover:bg-indigo-600 flex justify-center items-center">&laquo;</Link>
                    ) : null}
                    {pagination.pageNumbers.map((page, i) => (
                        <Link key={i} href={`/aktuelles${getNewSearchParams(filter, page)}`} className={`w-8 h-8 hover:bg-indigo-600 ${pagination.currentPage === page ? "bg-indigo-600" : ""} flex justify-center items-center`}>{page}</Link>
                    ))}
                    {pagination.pageNumbers[pagination.pageNumbers?.length - 1] !== pagination.currentPage ? (
                        <Link href={`/aktuelles${getNewSearchParams(filter, pagination.currentPage + 1)}`} className="w-8 h-8 hover:bg-indigo-600 flex justify-center items-center">&raquo;</Link>
                    ) : null}
                </div>
            ) : null}
        </Layout>
