...
export default function WantedPersonsFromProvince({ wantedPersons, query, pagination, params }) {
    const [filterToggled, setFilterToggled] = useState(false)
    const [filter, setFilter] = useState({
        suche: query.suche || "",
        tatzeit_von: query.tatzeit_von || "",
        tatzeit_bis: query.tatzeit_bis || "",
    ...
    const onProviceChange = (e) => {
        ...
        if (newProvince === "Deutschland") {
            router.push("/fahndungen" + urlSearchParams);
        } else {
            router.push("/fahndungen/" + newProvince + urlSearchParams);
        }
    }
    ...
    const submitSearch = () => {
        ...
        if (query.suche === filter.suche &&
            ...
            query.tatzeit_von === filter.tatzeit_von &&
            query.tatzeit_bis === filter.tatzeit_bis)
            return;

        router.push("/fahndungen/" + params.province + urlSearchParams);
    }

    return (
        <Layout 
            title={`Gesuchte Personen in ${params.province}`} 
            description="Gemeinsam Hinweise zur Fahndung geben, die Polizeiarbeit unterstützen und für mehr Sicherheit sorgen. Jeder Hinweis zählt!"
            canonical={`${process.env.NEXT_PUBLIC_FRONTEND_URL}/fahndungen/${params.province}`}
        >
                ...
                <div className={`max-w-2xl mx-auto w-full overflow-hidden ease-in-out duration-200 ${filterToggled ? "max-h-screen" : "max-h-0"}`}>
                    <div className="grid sm:grid-cols-2 gap-4 py-4 px-2 text-[#ddd]">
                        <div className="flex flex-col">
                            <label className="text-sm text-light-gray mb-1 ml-1" htmlFor="input__tatzeit-von">Tatzeit von</label>
                            <input type="date" id="input__tatzeit-von" value={filter.tatzeit_von} onChange={(e) => setFilter(prev => ({ ...prev, tatzeit_von: e.target.value }))} className="bg-dark-purple rounded-md px-2 py-1 placeholder:text-[#7374A4] outline-none focus:ring-1 ring-indigo-500" />
                        </div>
                        <div className="flex flex-col">
                            <label className="text-sm text-light-gray mb-1 ml-1" htmlFor="input__tatzeit-bis">Tatzeit bis</label>
                            <input type="date" id="input__tatzeit-bis" value={filter.tatzeit_bis} min={filter.tatzeit_von} onChange={(e) => setFilter(prev => ({ ...prev, tatzeit_bis: e.target.value }))} className="bg-dark-purple rounded-md px-2 py-1 placeholder:text-[#7374A4] outline-none focus:ring-1 ring-indigo-500" />
                        </div>
            ...
            <section className="container mx-auto mt-20 flex flex-wrap justify-center gap-6 px-4">
                {wantedPersons.length > 0 ? (
                    <>
                        {wantedPersons.map((p) => (
                            <div className="relative w-[250px] bg-white" key={p._id}>
                                <Link href={`/fahndungen/${p.crawledFrom}/${p._id}`} className="flex flex-col h-full">
                                    <div className="relative group h-full">
                                        ...
                                        <div className="opacity-0 invisible group-hover:opacity-100 group-hover:visible duration-300 absolute inset-0 px-4 py-5 bg-indigo-500 bg-opacity-80 flex flex-col">
                                            <p className="text-sm text-center font-semibold">{p.offenses.length > 0 ? p.offenses.join(", ") : "Klicke, um mehr zu erfahren"}</p>
                                    ...
                                    <div className="px-4 py-3 group flex-grow">
                                        ...
                                        <div className="flex">
                                            ...
                                            <p className="ml-1 text-gray-500 text-sm">{new Date(p.crimeSceneDate).toLocaleDateString("de-DE", { year: "numeric", month: "2-digit", day: "2-digit" })}</p>
                                        </div>
                                    </div>
                                </Link>
                    ...
                    <p className="text-gray-300 text-center">Zurzeit wird keine Person nach deiner Suche gefahndet.</p>
                )}
            </section>
            ...
            {wantedPersons.length > 0 ? (
                <div className="container mx-auto mt-10 flex gap-2 justify-center">
                    {pagination.currentPage !== 1 ? (
                        <Link href={`/fahndungen/${params.province}${getNewSearchParams(filter, pagination.currentPage - 1)}`} className="w-8 h-8 hover:bg-indigo-600 flex justify-center items-center">&laquo;</Link>
                    ) : null}
                    {pagination.pageNumbers.map((page, i) => (
                        <Link key={i} href={`/fahndungen/${params.province}${getNewSearchParams(filter, page)}`} className={`w-8 h-8 hover:bg-indigo-600 ${pagination.currentPage === page ? "bg-indigo-600" : ""} flex justify-center items-center`}>{page}</Link>
                    ))}
                    {pagination.pageNumbers[pagination.pageNumbers?.length - 1] !== pagination.currentPage ? (
                        <Link href={`/fahndungen/${params.province}${getNewSearchParams(filter, pagination.currentPage + 1)}`} className="w-8 h-8 hover:bg-indigo-600 flex justify-center items-center">&raquo;</Link>
                    ) : null}
                </div>
            ) : null}
        </Layout>
...
export async function getServerSideProps({ params, query }) {
    let wantedPersons = await fetchWantedPersons();

    if (!isValidProvince(params.province)) {
        params.province = "Deutschland"
    }

    wantedPersons = wantedPersons.filter(p => {
        let isValid = true;
        ...
        if (query.tatzeit_von && isValid) {
            const fromDate = parseDate(query.tatzeit_von, "yyyy-mm-dd", "-")
            if (fromDate !== NaN) {
                isValid = new Date(p.crimeSceneDate).getTime() > fromDate.getTime()
            }
        }

        if (query.tatzeit_bis && isValid) {
            const tillDate = parseDate(query.tatzeit_bis, "yyyy-mm-dd", "-")
            if (tillDate !== NaN) {
        ...
        if (query.suche && isValid) {
            ...
            if (!conditions.some(el => p.offenses?.includes(el)) &&
                !conditions.some(el => p.crimeSceneLocation?.includes(el)) &&
    ...
    const pagination = calculatePagination(wantedPersons.length, itemsPerPage, query.seite)
    wantedPersons = wantedPersons.slice(pagination.offset, pagination.offset + itemsPerPage)
