...
export default function MissingPersonsFromProvince({ missingPersons, query, pagination, params }) {
    ...
    const submitSearch = () => {
        ...
        router.push("/vermisst/" + params.province + urlSearchParams);
    }

    return (
        <Layout 
            title={`Vermisste Personen in ${params.province}`} 
            ...
            canonical={`${process.env.NEXT_PUBLIC_FRONTEND_URL}/vermisst/${params.province}`}
        >
            <section className="container mx-auto flex flex-col items-center pt-40 md:pt-52 px-4">
                <div className="relative max-w-2xl w-full">
                    ...
                    <div className="absolute bottom-0 top-0 -right-1">
                        <ProvinceSelect onProviceChange={onProviceChange} province={params.province} />
                    </div>
                </div>
            ...
            {missingPersons.length > 0 ? (
                <div className="container mx-auto mt-10 flex gap-2 justify-center">
                    {pagination.currentPage !== 1 ? (
                        <Link href={`/vermisst/${params.province}${getNewSearchParams(filter, pagination.currentPage - 1)}`} className="w-8 h-8 hover:bg-indigo-600 flex justify-center items-center">&laquo;</Link>
                    ) : null}
                    {pagination.pageNumbers.map((page, i) => (
                        <Link key={i} href={`/vermisst/${params.province}${getNewSearchParams(filter, page)}`} className={`w-8 h-8 hover:bg-indigo-600 ${pagination.currentPage === page ? "bg-indigo-600" : ""} flex justify-center items-center`}>{page}</Link>
                    ))}
                    {pagination.pageNumbers[pagination.pageNumbers?.length - 1] !== pagination.currentPage ? (
                        <Link href={`/vermisst/${params.province}${getNewSearchParams(filter, pagination.currentPage + 1)}`} className="w-8 h-8 hover:bg-indigo-600 flex justify-center items-center">&raquo;</Link>
                    ) : null}
                </div>
            ) : null}
        </Layout>
